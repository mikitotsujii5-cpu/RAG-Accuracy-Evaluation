...
> [!NOTE]
> 現行sourceはasset version `1.4.6`です。Python 311件とChat UI 32件の自動テスト、ローカル画面でPhase横並び、進捗、完了、停止を確認しました。直前のremote実測はasset `1.4.5`で、deployment `SUCCEEDED`、App `RUNNING`、compute `ACTIVE`、health HTTP 200、resource binding 7件、既存Index Variant一覧、AI Search 10件取得、LLM回答、MLflow Trace、PDFリンク引用まで確認済みです。asset `1.4.6`のremote実測は、デプロイ後に結果を記録します。
...
状態確認が一時的に失敗しても、評価自体を止めずに自動で再接続します。ページを再読込した場合や、別ページ／別Projectへ移動して戻った場合も、現在のProjectに実行中の評価があれば進捗を復元します。Lakeflow JobがNotebook開始前に失敗または停止した場合も、そのJob状態を使って画面を終端状態へ更新するため、待機中のまま残りません。
...
評価を止める場合は「評価を停止」を押します。停止要求はDelta Tableへ保存され、Lakeflow Job自体にも取消を依頼します。画面はすぐに監視をやめず、「停止」または別の終端状態へ確定するまで進捗を確認します。停止済みrunと完了runを同じ集計として扱わないでください。

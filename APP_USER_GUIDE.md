...
> [!NOTE]
> 現行sourceはasset version `1.5.0`です。PDF全画面viewerを含むUIテスト40件に合格し、field-eng-eastへのremote配置、health、Resource Binding 7件を確認済みです。Python 344件はasset `1.4.9`の確認記録です。
...
このページは、同じ質問をPhase 1〜5へ実行し、検索再現率、回答正解率、応答時間の変化を比較します。
...
- 「受付 → Job起動 → Phase評価 → 指標集計 → 改善提案」の工程。
- 完了試行数／全試行数、進捗割合、経過時間。
...
状態確認が一時的に失敗しても、評価自体を止めずに自動で再接続します。評価履歴から実行中runへ戻る最初の状態確認も、1回25秒でtimeoutし、spinnerを保ったまま最大3回再接続します。「Jobの状態を再確認しています」と表示される`UNKNOWN`状態は、一時的にLakeflow Jobの状態を取得できないという意味で、評価失敗ではありません。ページを再読込した場合や、別ページ／別Projectへ移動して戻った場合も、現在のProjectに実行中の評価があれば進捗を復元します。一方、Job ID、Job設定、権限などDatabricksが確定的に拒否した内容は`FAILED`として終了するため、待機中のまま残りません。Lakeflow JobがNotebook開始前に失敗または停止した場合も、そのJob状態を使って画面を終端状態へ更新します。

Lakeflow Jobが終了しても、指標とAI改善提案の取得が終わるまではspinnerと「結果取得中」を表示します。結果APIは1回45秒で応答を確認し、通信断や一時的なエラーでは最大3回まで自動で再試行します。それでも結果を取得できない場合、評価run自体は消えません。「評価履歴」の「更新」を押し、同じrunを選び直すと保存済み結果を再取得できます。
...
| 指標 | 初心者向けの意味 |
|---|---|
| 検索再現率（Recall@10） | 正解ページを上位10件の中でどれだけ見つけたか |
| Precision@10 | 上位10件のうち、正解ページがどれだけ含まれるか |
| nDCG@10 | 関連度の高いページを上位へ並べられたか |
| 回答正解率（Answer Correctness） | 期待する事実を回答に含めたか。期待回答・期待事実がないケースは`NULL`で平均から除外 |
| 根拠一致率（Groundedness） | 回答が検索した根拠に基づいているか |
| 回答時間 p50 | 回答完了までの応答時間の中央値 |
| Error rate | 全試行のうち失敗した割合 |

回答品質だけでなく応答時間も確認します。たとえばPhase 4で順位が改善しても、Rerankingの追加時間が要件を超える場合があります。
...
評価を止める場合は「評価を停止」を1回押します。画面はクリック直後に最新状態を確認し、停止APIの応答が一時的に失われた場合も最大3回まで再確認します。停止要求はDelta Tableへ先に保存され、Job登録と停止が同時になっても、同じ`eval_run_id`で見つけたLakeflow Jobへ取消を依頼します。停止POSTを待っている間にstatus監視が完了・失敗・停止を先に確認した場合は、遅い停止POSTを中止し、確認済みの状態と結果をそのまま表示します。すでに完了していた場合は、完了状態を停止で上書きせず「停止する必要はありません」と表示します。画面はすぐに監視をやめず、「停止」または別の終端状態へ確定するまで進捗を確認します。停止済みrunと完了runを同じ集計として扱わないでください。
...
- 現行asset `1.5.0`はGitHubの`main/app`からfield-eng-eastへ配置し、deployment `SUCCEEDED`、App `RUNNING`、compute `ACTIVE`、health version `1.5.0`／`databricks_ready=true`、resource binding 7件、PDF全画面viewerを確認しました。
...
| 「評価の受付結果を確認しています」が表示される | 通信の応答が途切れたため、同じ受付番号で安全に確認しています。開始を押し直さず、spinnerと工程表示が更新されるまで待つ |
| 「Jobの状態を再確認しています」が表示される | Lakeflow Jobの状態を一時的に取得できていません。失敗確定ではないため、再読み込みや新規開始をせず自動再確認を待つ。長時間続く場合は管理者へJob設定と権限を確認してもらう |
| 「評価を停止」がすぐ完了しない | 停止要求は保存済みで、停止APIとJob状態を自動再確認しています。ボタンを連打せず、停止・失敗・完了のいずれかに確定するまで待つ |
| 「結果取得中」が長く続く／結果取得エラーになる | 指標とAI改善提案を1回45秒、最大3回まで自動取得しています。評価を再実行せず、「評価履歴」を更新して同じrunを選び直す |

...
| 1 | [ローカル成果物を検証](01_local_validation.md) | asset `1.8.0`のPython 344件とUI 65件、合計409件を`SUCCESS` |
...
| 6 | [AI Searchを作成・検索確認](06_ai_search.md) | 現行の登録済みStandard／512 Profileを`SUCCESS`。非車両動的IndexとPDF削除後の個別後継Indexは旧方式の検証履歴 |
...
| 8 | [MLflowとDatabricks Appを配置](08_mlflow_and_app.md) | asset `1.7.0`をfield-eng-eastへ配置し、deployment／App／compute／binding 7件とStandard／512／Qwen3の1 Profile表示を`SUCCESS` |
| 9 | [アプリをend-to-end確認](09_end_to_end.md) | asset `1.4.7`で実行済みのPhase 1 runをasset `1.4.8`で再表示し`SUCCESS`。SSO済みremote 4画面、TTFT、追加Profileの手動登録確認は`PENDING` |

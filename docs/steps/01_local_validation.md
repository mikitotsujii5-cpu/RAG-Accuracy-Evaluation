...
`SUCCESS`。現行source asset `1.5.1`はPDF全画面viewerを含むUIのjsdom回帰テスト40件と差分checkに合格しました。Python 344件はasset `1.4.9`の確認記録です。
...
現行source asset `1.5.1`はUI 40件を確認済みです。field-eng-eastへのremote配置も`SUCCESS`です。Python 344件はasset `1.4.9`の確認記録です。

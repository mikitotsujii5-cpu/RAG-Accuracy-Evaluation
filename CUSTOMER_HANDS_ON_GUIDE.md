...
8. コンピュート起動中または環境準備中は再度開始ボタンを押さずに待ちます。`Lakeflow Jobを開く`が表示された場合は、同じrunの詳細を別タブで確認します。
9. 検索再現率、検索適合率、検索順位、回答正解率、根拠一致率、引用正解率、回答時間、トークン、エラー率を比較します。
10. PhaseごとのLLM改善提案とMLflow Traceを確認します。
11. 実行中に一度ページを再読込し、同じProjectへ戻ると進捗が復元されることを確認します。この確認を行う場合も開始ボタンを再度押しません。
12. 停止を試す場合は別の検証runで`評価を停止`を押し、Lakeflow Jobへの停止要求後も終端状態まで監視されることを確認します。正式比較用runは停止しません。
13. 時間に余裕があれば繰り返し回数を3へ上げ、同じ3問・同じ条件で再実行します。
...
> 撮影箇所：正解付き3問、横並びのPhase 1～5、`15試行`、進捗カード、主要3指標、改善提案。
> 合格表示：3問が`回答・検索の正解あり`で、`受付 → Job起動 → Phase評価 → 指標集計 → 改善提案`の状態と、検索再現率・回答正解率・回答時間を確認できる。

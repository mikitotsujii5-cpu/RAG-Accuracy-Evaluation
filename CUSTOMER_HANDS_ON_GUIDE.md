...
補足として、AI SearchのIndex GET応答は、全source列を同期するIndexで`columns_to_sync`を省略することがあります。App／Jobはこの省略だけをエラーにせず、source schemaと検索結果で必要列を確認します。応答に`columns_to_sync`または`columns_to_index`が表示される場合は、上記の必要列と完全一致していることを確認します。
...
`PREP_JOB_ID`と`EVAL_JOB_ID`には、Databricks UIで確認した数字だけを入力します。前後の空白、先頭ゼロ、URL、Job名は入れません。Appは正の整数でない値を起動時に拒否します。
...
1. `RAGチャット`を開きます。
2. 空の会話に9件の質問例が表示されることを確認し、1件を押して入力欄へ入れます。
3. 画面上部の`AI Search ↗`などを押し、同じWorkspaceの対応画面が新しいタブで開くことを確認します。
4. Vector Searchを選んで質問します。
5. 次にHybrid Searchを選んで同じ質問を送ります。
6. PDFリンクとページが表示されることを確認します。
7. 停止ボタンを押すと回答生成が止まることを確認します。
...
3. `比較条件を選択`で、横並びのPhase 1～5をすべて選びます。画面幅が狭い場合はPhaseカードを横へスクロールします。
4. 比較する検索データ、回答用LLM、採点用LLMを選びます。すべての条件がそろうまで開始ボタンが無効であることも確認します。
5. `繰り返し回数=1`のまま、実行数が`15試行`（3問 × 5 Phase × 1回）と表示されることを確認します。
6. `選択した3問でRAG精度を比較`を押します。クリック直後から回転表示と進捗カードが現れることを確認します。
7. `受付 → Job起動 → Phase評価 → 指標集計 → 改善提案`の工程、完了試行数／全試行数、割合、経過時間、Phase別状態を確認します。
8. コンピュート起動中、環境準備中、または`評価の受付結果を確認しています`と表示された場合は、再度開始ボタンを押さずに待ちます。通信が途切れても同じ`Idempotency-Key`（受付番号）で安全に再確認するため、評価runやJobは二重に作られません。`Lakeflow Jobを開く`が表示された場合は、同じrunの詳細を別タブで確認します。
9. Job終了後も「結果取得中」のspinnerが消えるまで待ちます。指標取得は1回45秒、最大3回まで自動再試行されます。取得エラーになった場合は評価を再実行せず、「評価履歴」を更新して同じrunを選び直します。
10. 検索再現率、検索適合率、検索順位、回答正解率、根拠一致率、引用正解率、回答時間、トークン、エラー率を比較します。
11. PhaseごとのLLM改善提案とMLflow Traceを確認します。
12. 実行中に一度ページを再読込し、同じProjectへ戻ると進捗が復元されることを確認します。この確認を行う場合も開始ボタンを再度押しません。最初の状態確認が遅い場合も、1回25秒のtimeoutと最大3回の自動再接続中はspinnerが続きます。`Jobの状態を再確認しています`と表示された場合は、一時的な`UNKNOWN`状態から自動回復するまで待ちます。
13. 停止を試す場合は別の検証runで`評価を停止`を1回押します。停止APIの応答が一時的に失われても自動再確認し、Lakeflow Jobへの停止要求後も終端状態まで監視されることを確認します。停止POSTより先に完了などの終端状態を確認した場合は、遅い停止POSTを中止して結果表示を維持します。正式比較用runは停止しません。
14. 時間に余裕があれば繰り返し回数を3へ上げ、同じ3問・同じ条件で再実行します。
...
> 撮影箇所：質問例9件、回答、PDFリンク、ページ、検索方式、`↗`付きDatabricks機能名。
> 合格表示：根拠がチャンク文字列ではなくPDFリンクで表示され、機能名から同じWorkspaceのコンソールを開ける。

> **スクショポイント SS-22C：RAG精度評価**
> 撮影箇所：正解付き3問、`比較条件を選択`、横並びのPhase 1～5、`15試行`、進捗カード、`結果取得中`、主要3指標、改善提案。
> 合格表示：3問が`回答・検索の正解あり`で、`受付 → Job起動 → Phase評価 → 指標集計 → 改善提案`の状態、結果取得中のspinner、検索再現率・回答正解率・回答時間を確認できる。
> 写さないもの：Workspace ID、利用者メールアドレス、Job／Deployment／その他Databricksリソースの実ID。必要な部分だけを切り取り、値はマスクする。
...
| 評価の受付確認が長い | AppからJob受付結果を一時取得できない | RAG精度評価の進捗／Evaluation Job | 開始を押し直さず自動再確認を待つ。長時間続く場合は、正の整数のEvaluation Job IDとApp SPの権限を確認する |
| Job状態が`UNKNOWN`と表示される | Jobs APIの状態取得が一時失敗した | RAG精度評価の進捗／Job run | 失敗確定ではないためspinnerのまま自動再確認を待つ。Job設定・権限の確定エラーなら画面は`FAILED`へ終了する |
| 評価停止の応答が遅い | 停止APIまたはJob状態確認の応答が一時的に失われた | RAG精度評価の進捗／Job run | 停止を連打しない。保存済み停止要求と同じ`eval_run_id`で自動再確認され、終端状態へ収束する |
| 評価は終わったが結果が出ない | 結果APIの応答が遅い、または一時失敗した | RAG精度評価の`結果取得中`／評価履歴 | 45秒timeoutで最大3回自動再試行するまで待つ。失敗後は評価履歴を更新し、同じrunを選んで保存済み結果を再取得する |
